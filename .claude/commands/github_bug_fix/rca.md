...
This RCA document should be used by `/github_bug_fix:implement-fix` command.

## Next Steps

1. Review this RCA document
2. Run: `/github_bug_fix:implement-fix $ARGUMENTS` to implement the fix
3. Ask whether context needs updating — a root cause worth an RCA is often
   worth a `docs/LESSONS.md` entry (`docs/CONTEXT-PROTOCOL.md`)
4. Run: `/commit` after implementation is complete
```
