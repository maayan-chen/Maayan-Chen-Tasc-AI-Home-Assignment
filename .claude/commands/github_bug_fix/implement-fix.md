...
allowed-tools: Read, Write, Edit, Bash(git:*), Bash(gh:*), Bash({{TEST_RUNNER}}:*), Bash({{PACKAGE_MANAGER}}:*)
---
...
**Test implementation** — follow the project's existing test framework and
conventions. The shape, whatever the language:

```
test "issue #$ARGUMENTS is fixed":
...
**Confirm the test has teeth:** revert the fix and watch the new test go red.
A regression test that passes against the unfixed code prevents nothing.
...
**First, ask whether context needs updating** (`docs/CONTEXT-PROTOCOL.md`). A
bug that warranted an RCA often warrants a `docs/LESSONS.md` entry — especially
if it passed every automated check, or if the obvious fix would have been wrong.
Propose as a checklist and wait for approval.
...
- If the RCA document is missing or incomplete, request it be created first with `/github_bug_fix:rca $ARGUMENTS`
